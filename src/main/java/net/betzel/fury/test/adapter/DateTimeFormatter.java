...
import static net.betzel.fury.test.adapter.DateTimeFormatterUtils.DATETIME_DELIMETER_LENIENT_MAP;
import static net.betzel.fury.test.adapter.DateTimeFormatterUtils.parseDualAttempt;
 ...
 * {@code xs:time} are {@link OffsetDateTime} and {@link OffsetTime}.
...
public class DateTimeFormatter {
     ...
     * <ol>
     *    <li>Use the static {@link #parseIntoOffsetDateTime(String, ZoneOffset) parse()} method on this class.
     ...
     *             If this is a problem then {@link #parseIntoOffsetDateTime(String, ZoneOffset) parseIntoOffsetDateTime()}
     *             can be used.</li>
     *      </ul>
     * </ul>
     *
     ...
     */
    public static final java.time.format.DateTimeFormatter XSD_DATETIME_PARSER;
     ...
     *        a {@link OffsetDateTime} or {@link LocalDateTime} is used as input. </li>
     ...
     * @see <a href="https://www.w3.org/TR/xmlschema-2/#dateTime">https://www.w3.org/TR/xmlschema-2/#dateTime</a>
     */
    public static final java.time.format.DateTimeFormatter XSD_DATETIME;
     ...
     *           <ul>
     *             <li>{@link #parseIntoOffsetDate(String, ZoneOffset) parseIntoOffsetDate()} </li>
     *             <li>{@link #parseIntoOffsetDateTimeNoTime(String, ZoneOffset)  parseIntoOffsetDateTimeNoTime()}</li>
     *           </ul>
     *        on this class.</li>
     *    <li>Creating a new {@code DateTimeFormatter} based on this one, but with a default ZoneId
     *        which is used during parsing if none is provided in the input.
     ...
     */
    public static final java.time.format.DateTimeFormatter XSD_DATE_PARSER;
     ...
     *        assuming that a {@link OffsetDateTime} or {@link LocalDate} is used as input.
     *        </li>
     ...
     * @see <a href="https://www.w3.org/TR/xmlschema-2/#date">https://www.w3.org/TR/xmlschema-2/#date</a>
     */
    public static final java.time.format.DateTimeFormatter XSD_DATE;
     ...
     * depending on your scenario:
     * <ol>
     *    <li>Use the static {@link #parseIntoOffsetTime(String, ZoneOffset) parse()}
     *        on this class.
     *        This method gracefully handles the situation where there are more then 9 digits
     *        on the seconds fractional value.</li>
     *    <li>Creating a new {@code DateTimeFormatter} based on this one, but with a default ZoneId
     ...
     *             This implementation will throw {@link java.time.format.DateTimeParseException DateTimeParseException}
     *             if the input has more than 9 fractional digits.
     *             If this is a problem then {@link #parseIntoOffsetTime(String, ZoneOffset) parseIntoOffsetTime()}
     *             can be used.</li>
     *      </ul>
     * </ul>
     *
     ...
     */
    public static final java.time.format.DateTimeFormatter XSD_TIME_PARSER;
     ...
     *        for {@code xs:time} assuming that a {@link OffsetTime}
     *        or {@link LocalTime} is used as input. </li>
     *    <li>Parsing: Compliant with the formal definition for {@code xs:time}, except for:
     * <ul>
     *   <li>The seconds fraction field may contain no more than 9 digits.
     *       In the formal definition, the seconds fractional field has "arbitrary level of precision".
     ...
     * @see <a href="https://www.w3.org/TR/xmlschema-2/#time">https://www.w3.org/TR/xmlschema-2/#time</a>
     */
    public static final java.time.format.DateTimeFormatter XSD_TIME;
    ...
    /**
     * Parses a {@code xs:dateTime} string into an {@link OffsetDateTime}.
     ...
     *
     * <p>
     * The string is parsed using {@link DateTimeFormatter#XSD_DATETIME_PARSER}
     * which is forgiving and accepts minor deviations from the official specification.
     *
     * <p>
     * Furthermore, parsing is attempted in a
     * {@link DateTimeFormatterUtils#parseDualAttempt(java.time.format.DateTimeFormatter, String, java.time.temporal.TemporalQuery)  two-step procedure}:
     * If first parsing attempt fails the method will evaluate if the problem is
     * something which can likely be
     * {@link DateTimeFormatterUtils#fixTooManySecondsDecs(String, java.time.format.DateTimeParseException)  fixed}
    ...
    public static OffsetDateTime parseIntoOffsetDateTime(String str, Function<LocalDateTime, ZoneOffset> zoneOffsetProvider) {
        return parseDualAttempt(DateTimeFormatter.XSD_DATETIME_PARSER, str, (temporal) -> {
     ...
     * {@link #parseIntoOffsetDateTime(String, Function)}
     * method.
     *
     * @param str               the string to parse
    ...
    /**
     * Parses a {@code xs:date} string into an {@link OffsetUtcDate OffsetDate}.
     * For example a string on the form {@code "2018-12-03+01:00"} (value with
     * offset) or {@code "2018-12-03"} (value without offset).
     *
     * <p>
     * The string is parsed using {@link DateTimeFormatter#XSD_DATE_PARSER}
     * which is forgiving and accepts minor deviations from the official specification.
     *
     * @param str                the string to parse
     * @param zoneOffsetProvider a function which will be called if no zone offset
     ...
     * @throws java.time.format.DateTimeParseException if unable to parse the string
     */
    public static OffsetUtcDate parseIntoOffsetDate(String str, Function<LocalDate, ZoneOffset> zoneOffsetProvider) {
        return DateTimeFormatter.XSD_DATE_PARSER.parse(str, (temporal) -> {
     ...
     * {@link #parseIntoOffsetDate(String, Function)}
     * method.
     *
     * @param str               the string to parse
     * @param defaultZoneOffset the zone offset to use if none is present in the
     ...
     * @throws java.time.format.DateTimeParseException if unable to parse the string
     */
    public static OffsetUtcDate parseIntoOffsetDate(String str, ZoneOffset defaultZoneOffset) {
     ...
     * <p>
     * This is a convenience method for the more generalized
     * {@link #parseIntoOffsetDateTimeNoTime(String, Function)}
     * method.
     *
     * @param str               the string to parse
     * @param defaultZoneOffset the zone offset to use if none is present in the
    ...
    /**
     * Parses a {@code xs:time} string into an {@link OffsetTime}.
     * For example a string on the form {@code "10:15:30+01:00"} (value with
     * offset) or {@code "10:15:30"} (value without offset).
     *
     * <p>
     * The string is parsed using {@link DateTimeFormatter#XSD_TIME_PARSER}
     * which is forgiving and accepts minor deviations from the official specification.
     *
     * <p>
     * Furthermore, parsing is attempted in a
    ...
    public static OffsetTime parseIntoOffsetTime(String str, Function<LocalTime, ZoneOffset> zoneOffsetProvider) {
        return parseDualAttempt(DateTimeFormatter.XSD_TIME_PARSER, str, (temporal) -> {
     ...
     * <p>
     * This is a convenience method for the more generalized
     * {@link #parseIntoOffsetTime(String, Function)}
     * method.
     *
     * @param str               the string to parse
     * @param defaultZoneOffset the zone offset to use if none is present in the
